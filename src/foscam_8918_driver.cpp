    ...
    camera_info_manager_(new camera_info_manager::CameraInfoManager(nh_))
{
    ...
    reconfig_cb = boost::bind(&Foscam8918::configCallback, this, _1, _2);
    ...
    pnh.param("url_prefix", url_prefix_, std::string("http://"));
    pnh.param("frame_id",   frame_id_,   std::string("foscam"));
    ...
    ros::Timer timer = nh_.createTimer(ros::Duration(1.0/rate_), &Foscam8918::timerCallback, this);
...
bool Foscam8918::connectToCamera()
{
    ...
    const std::string video_stream_address = url_prefix_ + username_ + ":" + password_ + "@" + ip_address_ + ":" + port_ + "/" + url_suffix_;
...
void Foscam8918::timerCallback(const ros::TimerEvent& event)
{
    ...
    if (have_connection_)
    {
        ...
        camera_pub_.publish(cv_img_.toImageMsg(), ci);
    }
}
